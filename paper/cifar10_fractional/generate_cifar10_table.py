#!/usr/bin/env python3
"""
Generate LaTeX table from CIFAR-10 experiment results.

This script processes the CIFAR-10 summary data and creates a professional LaTeX table
showing performance metrics across different numerical precision configurations.
Based on generate_otflowlarge_table.py but adapted for CIFAR-10 classification results.
"""
...
def load_and_process_data(csv_file, results_dir=None):
    """Load CIFAR-10 summary data and process it for table generation."""
...
def create_latex_table(data, output_file, width_filter=None, seed_filter=None):
    ...
    for prec in available_precisions:
        count = 0
        for solver in ['rampde']:#['torchdiffeq', 'rampde']:
    ...
    tabular_content += "\\textbf{CIFAR-10} & " + " & " * (total_cols - 1) + " \\\\\n"
    ...
    caption_text = "CIFAR-10 image classification performance of high-precision neural ODEs (rampde)"
    ...
    caption_text += ". Results report rampde-only runs for the selected precision configurations."

    full_table = f"""% CIFAR-10 Experiment Results
% Generated automatically from summary_ode_cifar10_fractional.csv
...
"""
    ...
    with open(tabular_file, 'w') as f:
        f.write("% CIFAR-10 Experiment Results - Tabular Only\n")
        f.write("% Generated automatically from summary_ode_cifar10_fractional.csv\n\n")
...
def create_summary_statistics(data, output_dir, width_filter=None, seed_filter=None):
    ...
    summary_file = output_dir / 'cifar10_summary_statistics.csv'
...
def main():
    parser = argparse.ArgumentParser(description='Generate CIFAR-10 LaTeX table')
    parser.add_argument('--csv-file', type=str, default='./raw_data/ode_cifar10_fractional/summary_ode_cifar10_fractional.csv',
                       help='Path to CIFAR-10 summary CSV file')
    parser.add_argument('--results-dir', type=str, default='./raw_data/ode_cifar10_fractional',
                       help='Path to CIFAR-10 results directory (for test loss data)')
    parser.add_argument('--output-dir', type=str, default='./outputs/tab_cifar10_results',
    ...
    print(f"Loading CIFAR-10 data from {csv_file}")
    data = load_and_process_data(csv_file, results_dir)
    print(f"Loaded {len(data)} experiments")
    
    # Create LaTeX table
    output_file = output_dir / 'cifar10_results_table.tex'
    ...
    csv_output_file = output_dir / 'cifar10_results_table.csv'
