...
def load_raw_data_from_directories(raw_data_dir: Path, dir_pattern: str = '*', metric: str = 'train_loss'):
    ...
    all_dirs = [
        ...
        if d.is_dir() and (d.name.startswith('stl10_fractional_') or d.name.startswith('stl10_'))
    ]
    ...
    if dir_pattern == '*':
        ...
        full_pattern = f"stl10_fractional*{dir_pattern}"
        matching_dirs = [d for d in all_dirs if fnmatch.fnmatch(d.name, full_pattern)]
    
    if not matching_dirs:
        raise FileNotFoundError(f"No directories found matching pattern: stl10_fractional*{dir_pattern}")
...
Examples:
  ...
  # Filter by seed
  python plot_stl10_fractional_convergence.py --filter "*seed42*"
  
  # Filter by width
  python plot_stl10_fractional_convergence.py --filter "*width_128*"
  
  # Multiple filters (combine patterns)
  python plot_stl10_fractional_convergence.py --filter "*seed42*width_128*"
        """
    )
    parser.add_argument('--raw-data-dir', type=str,
                       default='raw_data/ode_stl10_fractional',
