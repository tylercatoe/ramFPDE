        ...
        with torch.no_grad():
            for k in range(N-1):
                ...
                if t.requires_grad:
                    ...
                    grads = torch.autograd.grad(
                        dz_idx, (z_idx, t_idx, dt_local), scaler.S * at_history[idx],
                        create_graph=False, allow_unused=True
                    )
                    da_ind, gtj, gdtj = grads
                    ...
                    gdtj2 = torch.sum(scaler.S * at_history[idx] * dz_idx, dim=-1)
                else:
                    gtj = gdtj = gdtj2 = None
                ...
                if grad_t is not None and gdtj2 is not None:
                    ...
                    grad_t[idx].add_(scale_factor * (gtj - gdtj)).sub_(gdtj2_hi)
                    if idx + 1 < len(grad_t):
                        grad_t[idx + 1].add_(scale_factor  * gdtj).add_(gdtj2_hi)
